...
```css
.hero-gradient { /* Deep navy to indigo */ }
.accent-gradient { /* Electric blue to purple */ }
.card-glow { /* Radial glow at top */ }
.progress-gradient { /* Blue to purple */ }
...
<div className="rounded-2xl border p-6" style={{
  ...
  <div className="pointer-events-none absolute inset-0 rounded-2xl"
    style={{ background: 'radial-gradient(circle at 50% 0%, rgba(0, 201, 255, 0.08) 0%, transparent 60%)' }}
  />
  {/* Content */}
</div>
```
...
```tsx
<button className="accent-gradient text-white rounded-lg px-6 py-3"
